/**
 * Audio Duration Detection API for Vercel Edge Functions - URL Version
 * 支持通过URL获取远程音频文件的时长
 */
...
async function handleAudioDurationFromUrl(request) {
  try {
    ...
    if (!isValidAudioUrl(audioUrl)) {
      return new Response(JSON.stringify({
        error: 'Invalid URL',
        message: 'Invalid audio URL format or unsupported file extension'
      }), {
        status: 400,
        headers: {
          'Content-Type': 'application/json',
    ...
    // 下载并处理远程音频文件
    const audioData = await downloadRemoteAudio(audioUrl);
    const duration = await getAudioDurationFromBuffer(audioData.buffer, audioData.mimeType);
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...
        fileSize: formatFileSize(audioData.buffer.byteLength),
    ...
    console.error('Audio URL processing error:', error);
    
    // 根据错误类型返回不同的状态码
    let status = 500;
    let errorMessage = error.message || 'Failed to process audio URL';
    
    if (error.message.includes('timeout') || error.message.includes('fetch')) {
      status = 408; // Request Timeout
      errorMessage = 'Failed to download audio file: connection timeout or network error';
 ...
 * 下载远程音频文件
 */
async function downloadRemoteAudio(url) {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'AudioDurationAPI/1.0'
      }
    });
    ...
    if (contentLength && parseInt(contentLength) > maxSize) {
      throw new Error(`File too large: ${formatFileSize(parseInt(contentLength))} exceeds ${formatFileSize(maxSize)} limit`);
    }
    ...
    if (arrayBuffer.byteLength > maxSize) {
      throw new Error(`File too large: ${formatFileSize(arrayBuffer.byteLength)} exceeds ${formatFileSize(maxSize)} limit`);
    }
    ...
    if (error.name === 'AbortError') {
      throw new Error('Download timeout: failed to download audio file within 30 seconds');
    }
    throw error;
  }
}
...
function extractFilenameFromUrl(url) {
  try {
    ...
    return filename || 'remote-audio-file';
  } catch {
    return 'remote-audio-file';
  }
}
