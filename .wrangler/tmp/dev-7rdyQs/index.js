...
// .wrangler/tmp/bundle-aRZsG4/checked-fetch.js
...
// .wrangler/tmp/bundle-aRZsG4/strip-cf-connecting-ip-header.js
...
async function parseMp3Duration(arrayBuffer) {
  ...
  for (let i = dataStart; i < view.byteLength - 4; i++) {
    if (view.getUint8(i) === 255 && (view.getUint8(i + 1) & 224) === 224) {
      ...
      if (frameInfo && frameInfo.bitrate > 0 && frameInfo.sampleRate > 0) {
        const audioDataSize = arrayBuffer.byteLength - dataStart;
        const frameSize = frameInfo.frameSize;
        const totalFrames = Math.floor(audioDataSize / frameSize);
        const samplesPerFrame = frameInfo.samplesPerFrame;
        const duration = totalFrames * samplesPerFrame / frameInfo.sampleRate;
        return duration;
      }
    }
  }
...
// .wrangler/tmp/bundle-aRZsG4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aRZsG4/middleware-loader.entry.ts
